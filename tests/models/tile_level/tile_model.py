...
def _test_forward():
    ...
    assert (
        "tissue_segmentation" in outputs
    ), "Expected output to include 'tissue_segmentation' key."
    assert isinstance(
        outputs["tissue_segmentation"], torch.Tensor
    ...
    assert isinstance(
        outputs["hematoxylin"], torch.Tensor
